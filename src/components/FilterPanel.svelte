<script lang="ts">
  ...
  import { countModes, countStatuses, eventModeLabel } from '$lib/filter';
  import { EVENT_MODES } from '$lib/snapshot-types';
  ...
  const modeCounts = $derived(countModes(rows));
  const hasTagCounts = $derived(SCHOOL_TAGS.some((tag) => (tagCounts.get(tag) ?? 0) > 0));
  ...
  function isOnMode(value: string) {
    return filters.modes.includes(value as never);
  }
...
<div aria-label="筛选条件" class="surface-1 border border-line rounded-xl overflow-hidden {mode === 'sidebar' ? 'sticky top-[68px] max-h-[calc(100dvh-84px)] overflow-y-auto' : ''}">
  ...
  <div class="px-4 py-3 border-b border-line">
    <div class="flex items-center justify-between mb-2">
      <h3 class="text-fg-3 text-[11px] uppercase tracking-[0.16em] font-medium">形式</h3>
      {#if filters.modes.length > 0}
        <button
          class="text-fg-3 hover:text-fg-1 text-[11px]"
          onclick={() => (filters.modes = [])}
        >清除</button>
      {/if}
    </div>
    <div class="flex flex-wrap gap-1.5">
      {#each EVENT_MODES as eventMode}
        {@const on = isOnMode(eventMode)}
        {@const c = modeCounts[eventMode]}
        <button
          disabled={c === 0 && !on}
          onclick={() => (filters.modes = toggle(filters.modes, eventMode))}
          aria-label="筛选形式：{eventModeLabel(eventMode)}"
          aria-pressed={on}
          class="inline-flex items-center gap-1.5 text-xs rounded-md px-2.5 py-1.5 transition border
            {on
              ? 'bg-violet-100 border-violet-300 text-violet-800 dark:bg-violet-500/15 dark:border-violet-500/40 dark:text-violet-200'
              : 'surface-2 hover:surface-3 border-line text-fg-1 disabled:text-fg-4 disabled:hover:surface-2 disabled:cursor-not-allowed'}"
        >
  ...
  <div class="px-4 py-3 {mode === 'drawer' ? '' : 'border-b border-line'}">
    <div class="flex items-center justify-between mb-2">
      <h3 class="text-fg-3 text-[11px] uppercase tracking-[0.16em] font-medium">院校所在地</h3>
