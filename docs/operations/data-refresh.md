...
- 每条新增项在 release decision/公开写入前通过 additions-only 的四段 `projectId`、`name`/`institute` 精确对应、Unicode `Cf`、检查副本 skeleton 的明确学校范围/系统/办公室标签及未限定研究生院门禁；该检查不重写字段证据，也不适用于历史父项或 replay；
