...
worker_title_is_exact() { python3 -c 'import pathlib,sys; value=pathlib.Path(sys.argv[1]).read_bytes().rstrip(b"\0"); sys.exit(0 if value == b"nginx: worker process" else 1)' "$1"; }
collect_valid_workers() {
  local status_path pid parent_pid worker_exe
  for status_path in "$PROC_ROOT"/[0-9]*/status; do
    ...
    worker_title_is_exact "$PROC_ROOT/$pid/cmdline" || continue
